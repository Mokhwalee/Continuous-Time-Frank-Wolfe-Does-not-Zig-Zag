...
def FW(objFun, dFun, dFun_mom, m, alpha, T, n, disc_type, line_search = False, momentum = False, tol=1e-8):
    x_t = np.zeros((m, 1))
    dt = T/(n+0.)
    oracles  = np.zeros(n)
    trace = np.zeros(n)
    ...
    for it in range(n):
        ...
        if momentum:
            d,grad, theta,y,v = dFun_mom(x_t,alpha,theta,y,v,betaFun(i*dt))
...
def FW_matfact(objFun, dFun, dFun_mom, dim, alpha,rho, T, n, disc_type, line_search = False, momentum = False, tol=1e-8):
    ...
    for it in range(n):
        ...
        if momentum:
            d,grad, theta,y,v = dFun_mom(x_t,alpha,rho,theta,y,v,betaFun(i*dt))
